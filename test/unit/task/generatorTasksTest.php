...
$t->task_ok('sfGenerateTestTask', array('BarForm'), array(), false, '"sfGenerateTestTask" fails if test script exists');
$t->task_ok('sfGenerateTestTask', array('BarForm'), array('--force'), true, '"sfGenerateTestTask" succeeds with existing file and --force option');
$t->ok(file_exists(sfConfig::get('sf_test_dir').'/unit/form/BarFormTest.php'), '"sfGenerateTestTask" generates a test file');
$t->like(@file_get_contents(sfConfig::get('sf_test_dir').'/unit/form/BarFormTest.php'), '~'.preg_quote('dirname(__FILE__).\'/../../bootstrap/unit.php\'').'~', '"sfGenerateTestTask" includes a relative path to bootstrap');
